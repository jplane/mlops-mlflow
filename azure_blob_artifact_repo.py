...
class AzureBlobArtifactRepository(ArtifactRepository):
    ...
    def __init__(self, artifact_uri, client=None):
        ...
        try:
            ...
            raise Exception("'AZURE_STORAGE_CONNECTION_STRING' environment variable value not valid.")

    @staticmethod
    def parse_wasbs_uri(uri):
        """Parse a wasbs:// URI, returning (container, path)."""
        parsed = urllib.parse.urlparse(uri)
        if parsed.scheme != "wasbs":
            raise Exception("Not a WASBS URI: %s" % uri)

        match = re.match(
            r"([^@]+)@(([^.]+)\.(blob\.core\.(windows\.net|chinacloudapi\.cn|usgovcloudapi\.net))|azurite)", parsed.netloc
        )
    ...
    def log_artifact(self, local_file, artifact_path=None):
        (container, dest_path) = self.parse_wasbs_uri(self.artifact_uri)
    ...
    def list_artifacts(self, path=None):
        ...
        (container, artifact_path) = self.parse_wasbs_uri(self.artifact_uri)
        container_client = self.client.get_container_client(container)
    ...
    def _download_file(self, remote_file_path, local_path):
        (container, remote_root_path) = self.parse_wasbs_uri(self.artifact_uri)
        container_client = self.client.get_container_client(container)
